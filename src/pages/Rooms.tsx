...
const Rooms = () => {
  ...
  const openImageViewer = (room: typeof rooms[0], index: number) => {
    setSelectedRoom(room);
    ...
    >
      <div className="max-w-7xl mx-auto">
                  ...
                  {room.images.map((image, index) => (
                    <SwiperSlide key={index}>
                      <img
                        ...
                        className="w-full h-64 object-cover cursor-pointer"
                        onClick={() => openImageViewer(room, index)}
                      />
                    </SwiperSlide>
                  ))}
                  <div className="swiper-button-prev !text-white !opacity-0 group-hover:!opacity-100 transition-opacity"></div>
                  <div className="swiper-button-next !text-white !opacity-0 group-hover:!opacity-100 transition-opacity"></div>
                </Swiper>
              </div>
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
        ...
        {showImageViewer && selectedRoom && (
          ...
          >
            <div className="relative max-w-7xl mx-auto px-4 py-8" onClick={(e) => e.stopPropagation()}>
              <button
                className="absolute top-4 right-4 text-white p-2 hover:bg-white/10 rounded-full z-50"
                onClick={() => setShowImageViewer(false)}
              >
              ...
              <button
                className="modal-prev-button absolute left-8 top-1/2 -translate-y-1/2 text-white p-2 hover:bg-white/10 rounded-full z-50"
              >
                <ChevronLeft className="w-8 h-8" />
              </button>
              <button
                className="modal-next-button absolute right-8 top-1/2 -translate-y-1/2 text-white p-2 hover:bg-white/10 rounded-full z-50"
              >
                <ChevronRight className="w-8 h-8" />
              </button>
            </div>
